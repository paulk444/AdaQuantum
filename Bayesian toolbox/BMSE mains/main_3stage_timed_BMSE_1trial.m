...
%%================= AdaQuantum BMSE 1-trial ===============%%
%--- See User Guide for how to set up and run this code ----%

% This code is specifically designed for the FOM being the Bayesian Mean Squared Error (BMSE), for 1 trial (see https://arxiv.org/pdf/1812.01032.pdf)

% Stage 1: Randomly generate states, and keep states with a large QFI and with nbar = 0.5
% Stage 2: BMSE FOM, 1 TRIAL (nbar = 0.5)
% Stage 3: BMSE FOM, 1 TRIAL (nbar = 0.5)

% Load full toolbox
load('Full Toolbox.mat')

% Load selected toolbox
load('Bayes_tool_1_with1op.mat')
...
%% Options for stage 1
Truncation_initial = 60

%% Options for both ga's
ga_options.MutationFcn = {@mutationPower_Paul, 10}; % default 10 ?
% ga_options.MutationFcn = {@mutationPower_selection, 10, 0.3}; % default 10, 0.05 ?

% Crossover (selecting nothing chooses Matlab's default crossover function)
% ga_options.CrossoverFcn = @crossoversinglepoint;
% ga_options.CrossoverFcn = @crossovertwopoint;

%% Options for stage 2: intermediate pop
Population_size_s2 = 2400;
Cross_s2 = 0.3;
tourn_size1 = 8;
