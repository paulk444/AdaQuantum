...
function [Generated_DNA,Corresponding_FOM] = Randomly_generate_DNA_with_FOM_timed(SelectedToolbox, SelectedFigureOfMerit, use_heralding_probability, num_modes, max_ops, output_loss, DNA_length, LowerBounds, UpperBounds, Integers, Truncation_initial, heralding_accepted, Population_size_ga1, Time_p1_hrs, pop_chunk_size)

% Randomly generate DNA and its corresponding FOM
% Stop after a certain time, as long as there is enough DNA generated for phase 2
...
generated_pop_size = 0;
time_run = 0;
chunks_generated = 0;
tic

while generated_pop_size < Population_size_ga1 || time_run < Time_p1_hrs*3600

    Generated_DNA_chunk = zeros(pop_chunk_size,DNA_length);
    Corresponding_FOM_chunk = zeros(pop_chunk_size,1);
    
    parfor pop = 1:pop_chunk_size
        ...
        %% Save DNA and FOM
        Generated_DNA_chunk(pop,:) =  DNA';
        Corresponding_FOM_chunk(pop,1) = FigureOfMeritValue;

    end
    
    time_run = toc;
    chunks_generated = chunks_generated + 1;
    generated_pop_size = chunks_generated*pop_chunk_size;

    Generated_DNA((chunks_generated-1)*pop_chunk_size+1:chunks_generated*pop_chunk_size,:) = Generated_DNA_chunk;
    Corresponding_FOM((chunks_generated-1)*pop_chunk_size+1:chunks_generated*pop_chunk_size,1) = Corresponding_FOM_chunk;
