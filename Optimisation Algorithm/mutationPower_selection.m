function mutationChildren = mutationPower_selection(parents,options,GenomeLength,~,~,~,thisPopulation,mutPower,rate)
% mutationPower_select. Power mutation that only acts on SOME of the DNA elements. Adapted from MUTATIONUNIFORM (the latter is built in to Matlab)
%
...
for i=1:length(parents) % for each parent
    ...
    for j=1:GenomeLength % for each DNA element
        
        if rand < rate % Only mutate some of the DNA elements. E.g. if rate = 0.1 then only 1/10th of the DNA elements will be mutated
